...
def validate_sampling_parameters(shots: int, seed: Optional[int] = None) -> None:
    """
...
def format_simulation_results(
    num_fails: np.ndarray, shots: int, extra_outputs: dict, verbose: bool = False
) -> str:
    """
    ...
    if "stats" in extra_outputs:
        ...
        lines.append(
            f"95% CI: [{pfail_ci - delta_pfail:.6f}, {pfail_ci + delta_pfail:.6f}]"
        )
    ...
    if verbose and "fails" in extra_outputs:
        ...
        lines.append(
            f"Failure pattern: {np.sum(fails, axis=1)[:10]}..."
        )  # First 10 samples
...
def analyze_error_patterns(
    errors: np.ndarray, det_outcomes: np.ndarray, max_analysis_samples: int = 1000
) -> dict:
    """
    ...
    analysis = {
        ...
        "error_correlation": (
            np.corrcoef(np.sum(sample_errors, axis=1), np.sum(sample_det, axis=1))[0, 1]
            if n_samples > 1
            else 0.0
        ),
    }
