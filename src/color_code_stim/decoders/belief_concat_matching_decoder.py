...
class BeliefConcatMatchingDecoder(BaseDecoder):
    ...
    def decode(
        ...
        **kwargs
        ...
        for i, det_outcomes_single in enumerate(detector_outcomes):
            if verbose:
                print(
                    f"Processing sample {i+1}/{len(detector_outcomes)} with updated DEMs..."
                )

            # Step 4: Create updated DEMs using BP probabilities
            updated_dems = self._create_updated_dems(
                colors, bp_probs[i] if bp_probs.ndim > 1 else bp_probs
            )
        ...
        if full_output:
            ...
            for key in extra_outputs_list[0].keys():
                try:
                    aggregated_extra[key] = np.concatenate(
                        [eo[key] for eo in extra_outputs_list], axis=0
                    )
    ...
    def _create_updated_dems(
        self, colors: List[str], bp_probs: np.ndarray
    ...
    def _decode_with_updated_dems(
        ...
        **kwargs
        ...
        return self.concat_decoder.decode(
            detector_outcomes, custom_dem_data=updated_dem_data, **kwargs
        )
